package com.vholodynskyi.assignment.domain.use_case.insert_all_contacts_use_case
...
class InsertAllContactsUseCase @Inject constructor(private val repository: ContactRepository) {

    operator fun invoke(contactList: MutableList<DbContact>) = flow {
        try {
            emit(Resource.Loading<List<ApiContact>>())
            val result = repository.insertContacts(contactList)
